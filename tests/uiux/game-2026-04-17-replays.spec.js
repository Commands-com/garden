  ...
  return page.evaluate(async (scheduledPlacements) => {
    ...
    const placementOutcome = await new Promise((resolve) => {
      const step = () => {
        ...
        const nowMs = observation?.survivedMs || 0;
        if (nowMs >= lastSnapshotMs + 2000) {
          lastSnapshotMs = nowMs;
          placementSnapshots.push({
            atMs: nowMs,
            wave: state?.wave,
            gardenHP: state?.gardenHP,
            resources: state?.resources,
            enemies: (observation?.lanes || []).flatMap((lane) =>
              (lane.enemies || []).map((e) => ({
                id: e.enemyId,
                lane: lane.row,
                x: Math.round(e.x),
                hp: e.hp,
                slow:
                  e.statusEffects?.slow?.remainingMs > 0
                    ? e.statusEffects.slow.remainingMs
                    : 0,
              }))
            ),
            defenders: (observation?.lanes || []).flatMap((lane) =>
              (lane.plants || []).map((p) => ({
                row: lane.row,
                col: p.col,
                id: p.plantId,
        ...
        const occupied = Boolean(
          lane?.plants?.some(
            (candidate) => candidate.col === nextPlacement.col
          )
        );
    ...
    if (placementOutcome.reason === "gameover-before-placement") {
      return {
        ...
        placementSnapshots: placementOutcome.placementSnapshots,
      };
    }
    ...
    const terminalOutcome = await new Promise((resolve) => {
      const pollTerminal = () => {
        ...
        const nowMs = obs?.survivedMs || 0;
        if (nowMs >= snapshotTaken + 2000) {
          snapshotTaken = nowMs;
          snapshotTrail.push({
            atMs: nowMs,
            wave: state?.wave,
            enemies: (obs?.lanes || []).flatMap((lane) =>
              (lane.enemies || []).map((e) => ({
                id: e.enemyId,
                lane: lane.row,
                x: Math.round(e.x),
            ...
            defenders: (obs?.lanes || []).flatMap((lane) =>
              (lane.plants || []).map((p) => ({
                row: lane.row,
                col: p.col,
                id: p.plantId,
        ...
        if (state?.scene === "gameover") {
          resolve({ outcome: "gameover", state, obs });
          return;
        }
        if (state?.scenarioPhase === "endless" || state?.challengeCleared) {
          resolve({ outcome: "cleared", state, obs });
          return;
        }
        if (Date.now() - waitStart > terminalWaitMs) {
          resolve({ outcome: "timeout", state, obs });
          return;
        }
    ...
    return {
      ...
      finalObservation: terminalOutcome.obs,
      snapshotTrail,
    };
  }, placements);
}

test.describe("April 17 replay probes (natural outcomes)", () => {
  test("replay-2026-04-17-chilled-lane.json clears the challenge naturally", async ({
    page,
  }) => {
    test.setTimeout(120000);

    const runtimeErrors = await prepareGamePage(page);
    const fixture = readReplayFixture("replay-2026-04-17-chilled-lane.json");
    ...
    expect(
      result,
      `chilled-lane replay did not clear naturally: ${JSON.stringify(result, null, 2)}`
  ...
  test("replay-2026-04-17-no-control.json loses before clear without Frost control", async ({
    ...
    const runtimeErrors = await prepareGamePage(page);
    const fixture = readReplayFixture("replay-2026-04-17-no-control.json");
    ...
    expect(
      result,
      `no-control replay did not reach gameover: ${JSON.stringify(result, null, 2)}`
